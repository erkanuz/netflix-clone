...
const FavCard = ({ movie }: Props) => {
  ...
  const handleDelete = async () => {
    if (buttondelete) {
      ...
      toast.error(`${movie?.title} has been removed from My List`, { style: toastStyle });
    }
  };

  const buttondelete = true;
  ...
  return (
    <div className="relative h-28 min-w-[140px] transition duration-200 ease-out md:h-36 md:min-w-[260px] cursor-pointer group">
      ...
      <div className="absolute top-0 left-0 w-full h-full hover:bg-black/80 hover:rounded-sm opacity-0 hover:opacity-100 text-white">
        <p
          className="text-xs md:text-sm font-bold flex justify-center items-center h-full text-center whitespace-normal"
          onClick={() => {
        ...
        <div className="absolute right-2 bottom-2" onClick={handleDelete}>
          {buttondelete && <AiFillDelete />}
        </div>
      </div>
    </div>
  );
